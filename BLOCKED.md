...
- 无新沙箱拒绝发生（未触碰 ~/.dsh、未重启 dsh web、未触发提权弹窗）。
- 运维教训（供后续反向验证参考）：对**未提交的大文件**做反向验证（如清空 `PINYIN_SYLLABLE_TABLE`）时，不要用「移动备份再改」的流程——`Move-Item` 会消耗备份，源文件未提交时无法 `git checkout` 还原。**正确做法**：改前 `Copy-Item` 留副本，或先提交基线再注入坏值，改后 `git checkout` 还原。本轮曾因此需要手工重建 458 字音节表（已用 edit 精确复原，字节级核对无重复键）。
- 反向验证全程证据：拼音表清空 → 4 用例红（`expected [] to include '@linxin666/dsh-ssh'`，119/123）；还原 → 123/123 全绿。

### 新运行依赖（任务书要求记录）

- 无新增运行/开发依赖（pinyin/categories/plugin-manifest 三个新纯模块零外部 import；package.json 仅 `files` 加 `docs`）。
